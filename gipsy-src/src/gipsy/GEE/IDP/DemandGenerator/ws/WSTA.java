...
import gipsy.GEE.IDP.DemandGenerator.jini.rmi.MulticastJiniServiceDiscoverer;
...
public class WSTA implements Serializable, ITransportAgent{
	 ...
	 */
	public WSTA(Configuration poTAConfig) 
	{
		if(poTAConfig == null)
		{
			poTAConfig = new Configuration();
			poTAConfig.setProperty(ITransportAgent.TA_IMPL_CLASS, WSTA.class.getCanonicalName());
	...
	throws DMSException 
	{
		try
		{
			...
			WSDispatcherEntry oTemplate = new WSDispatcherEntry();
	...
	throws DMSException
	{
		try 
		{
			...
			WSDispatcherEntry oEntry;
			
			// Wait here until we get the value
			oEntry = new WSDispatcherEntry();
			oEntry.oSignature = poSignature;
			oEntry.oDemandState = DemandState.COMPUTED;
			
			if(pbIsPeeking)
			{
				oEntry = (WSDispatcherEntry)oJavaSpace.read(oEntry, null, JavaSpace.NO_WAIT);
			}
			else
			{
				oEntry = (WSDispatcherEntry)oJavaSpace.read(oEntry, null, Lease.FOREVER);
			}
	...
	throws DMSException
	{
		...
		try 
		{
			...
			oEntry = new WSDispatcherEntry(poDemand, poState);
			...
			if(poState.isComputed())
			{
				...
				WSDispatcherEntry oTemplate = 
					new WSDispatcherEntry(poDemand.getSignature(), DemandState.INPROCESS);
