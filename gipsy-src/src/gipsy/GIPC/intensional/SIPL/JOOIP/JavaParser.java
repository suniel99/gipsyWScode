...
import gipsy.GIPC.intensional.SIPL.JOOIP.ast.*;
import gipsy.GIPC.intensional.SIPL.JOOIP.ast.body.*;
import gipsy.GIPC.intensional.SIPL.JOOIP.ast.expr.*;
import gipsy.GIPC.intensional.SIPL.JOOIP.ast.stmt.*;
import gipsy.GIPC.intensional.SIPL.JOOIP.ast.type.*;
...
public final class JavaParser implements JavaParserConstants {
    ...
    public static JavaIdentifierSymbolTable oIdentifier; //add for recording all Java identifiers.
    public static Hashtable JavaIdentifierTable; //the table.
    public static Vector ClassList = new Vector();
    ...
    public static Vector parse(InputStream in) throws ParseException {
        parse(in, null);
        return ClassList;
    }
 ...
 */
  final public CompilationUnit CompilationUnit() throws ParseException {
    ...
    while (true) {
      ...
      tn = TypeDeclaration();
      //record type of class
      //System.out.println("test 0,for class name:"+tn.name+" "+tn.modifiers+" "+tn.members.size());
      if(line==-1){
    	   	  line = tn.getLine(); column = tn.getColumn();} types = add(types, tn);    	  
    }
    switch (jj_nt.kind) {
    case 0:
      jj_consume_token(0);
 ...
 */
  ...
  final public ClassOrInterfaceDeclaration ClassOrInterfaceDeclaration(Modifier modifier) throws ParseException {
   ...
   JavaClassSymbolTable temp_class;
   String name;
   List typePar = null;
   List extList = null;
   List impList = null;
   ...
   String temp_ext="";
   String temp_imp="";
   JavaIdentifierTable = new Hashtable();
    switch (jj_nt.kind) {
    case CLASS:
    ...
    {if (true) {
    	//when a class is done, return the class name with its extends and implements name and its member table;
    	if (!(extList == null)) temp_ext=extList.toString();
    	if (!(impList == null)) temp_imp=impList.toString();
    	//System.out.println("test for member record "+name+" "+JavaIdentifierTable.size());
    	temp_class = new JavaClassSymbolTable(name,temp_ext,temp_imp,JavaIdentifierTable); 
    	ClassList.addElement(temp_class);
  ...
  final public FieldDeclaration FieldDeclaration(Modifier modifier) throws ParseException {
    ...
    jj_consume_token(SEMICOLON);
    {if (true) {
    	//type mapping system from Java type to IPL type
    	if (type.toString().equals("char"))temp_type=4;
    	else if (type.toString().equals("byte")||type.toString().equals("short")||type.toString().equals("int"))temp_type=0;
    	else if (type.toString().equals("float")||type.toString().equals("double"))temp_type=1;
    	else if (type.toString().equals("boolean"))temp_type=3;
    	else if (type.toString().equals("String"))temp_type=2;
    	else {
    		temp_type = 6;
    		temp_ClassName = type.toString();
    	}
    	if (!(variables.toString().indexOf('=')==-1)){
    		temp_init=variables.toString().substring(variables.toString().indexOf('=')+1,variables.toString().length()-1).trim();
    		temp_vars=variables.toString().substring(1,variables.toString().indexOf('=')).trim();    		
    	} else
    	{
    		temp_vars=variables.toString().substring(1, variables.toString().length()-1).trim();
    	}
    	while (!fin){
    		//System.out.println("variables="+temp_vars);
    		if (temp_vars.toString().indexOf(',')==-1) {
    				temp_name=temp_vars.toString().trim();
    				//System.out.println("variables2="+temp_name);
    				oIdentifier = new JavaIdentifierSymbolTable(temp_name,true,temp_type,temp_ClassName,temp_init,null,null);
    				//System.out.println("members="+oIdentifier.Id);
    		    	JavaIdentifierTable.put(oIdentifier.strID, oIdentifier);
    				fin=true;
    		}
    		else{
    			temp_name=temp_vars.substring(0,temp_vars.indexOf(',')).trim();
    			//System.out.println("variables3="+temp_name);
				oIdentifier = new JavaIdentifierSymbolTable(temp_name,true,temp_type,temp_ClassName,temp_init,null,null);
		    	JavaIdentifierTable.put(oIdentifier.strID, oIdentifier);
				temp_vars=temp_vars.substring(temp_vars.indexOf(',')+1).trim();
    		}
    	}    	
    	// System.out.println("test 5 for type="+type+" variables="+variables+" ");
  ...
  final public VariableDeclarator VariableDeclarator() throws ParseException {
    ...
    {if (true) {//when it is identifier decalaration, record it. Change 1.
    	
    	//oIdentifier = new JavaIdentifierSymbolTable(id.toString(),"VAR",init,null);
    	//JavaIdentifierTable.put(oIdentifier.Id, oIdentifier);
    	
    	//VariableDeclarator temp = new VariableDeclarator(id.getLine(), id.getColumn(), id, init);
    	//System.out.println("test 3, for variable:"+temp.id+" "+temp.init);
  ...
  final public Parameter FormalParameter() throws ParseException {
    ...
    id = VariableDeclaratorId();
    {if (true) {
    	//when it is a parameter decalaration, record it too. Change 2
    	//type mapping system from Java type to IPL type
    	if (type.toString().equals("char"))temp_type=4;
    	else if (type.toString().equals("byte")||type.toString().equals("short")||type.toString().equals("int"))temp_type=0;
    	else if (type.toString().equals("float")||type.toString().equals("double"))temp_type=1;
    	...
    	oIdentifier = new JavaIdentifierSymbolTable(id.toString(),true,temp_type,temp_ClassName,null,null,null);
    	//oIdentifier = new JavaIdentifierSymbolTable(id.toString(),"PARA",null,null);
    	JavaIdentifierTable.put(oIdentifier.strID, oIdentifier);
    	
    	Parameter temp = new Parameter(type.getLine(), type.getColumn(), modifier.modifiers, modifier.annotations, type, isVarArg, id);
    	//System.out.println("test parameter "+temp.data+" "+temp.id+ " "+temp.isVarArgs+" "+temp.type);
    ...
    case LPAREN:
      ...
      case RUNSIGNEDSHIFTASSIGN:
        ...
        case RUNSIGNEDSHIFTASSIGN:
          op = AssignmentOperator();
          value = Expression();
          /* old design think to delete: 
           * when it is an assignment, check the table, if there is no initial value for a variable, replace by this assignment. Change 3
           * if (JavaIdentifierTable.containsKey(expr.toString()))
           *{
        *	  JavaIdentifierSymbolTable temp = (JavaIdentifierSymbolTable)JavaIdentifierTable.get(expr.toString());
        *	  if (temp.Init.equals(null)) temp.Init = value.toString();
        *      JavaIdentifierTable.put(expr.toString(), temp);
        *          }
        *  System.out.println("test 6: expr="+expr+"value= "+value+" op="+op);*/
...
/* Annotation Types. */
  ...
  final private Token jj_consume_token(int kind) throws ParseException {
    ...
    //first check if the token is IPL identifier;
    if (jj_nt.toString().contains("IPL_CODE"))
    {    	
    	oIdentifier = new JavaIdentifierSymbolTable(jj_nt.image, false, -1, "", "", null, null);
		//System.out.println("members="+oIdentifier.Id);
    	JavaIdentifierTable.put(oIdentifier.strID, oIdentifier);
    	//System.out.println(" ="+jj_nt.image);
    }
    if (token.kind == kind) {
      jj_gen++;
      if (++jj_gc > 100) {
