...
public class DFGTranCodeGenerator
{
  ...
  Stack SKrankmax = new Stack(); // some node for max rank together
  Stack SKrankmin = new Stack(); // some node for min rank together
  private String lname = "0"; // "Y X" full node name with scope by space ; start from "0"
  private String sname = ""; // "X" short node name
  //    private DFGSubtree SBTstru; // simble table structure: nterm| used flag
  //    private Hashtable SBT;  // simble table : long name| its structure
  DFGSubtree sbt = new DFGSubtree();
  private int scope = 0;
  ///////// for cyc 1
  private int scope0 = 0;
  private String lname0 = "0";
  private Hashtable ntable = new Hashtable(); // store name list-("name",Int)
  private Hashtable dtable = new Hashtable(); // store dimesion name ("name","status")
  private int nterm = 0;
  Stack SKtoname = new Stack();
  ...
  private String flagdim = ""; // if dimension begin then = dim or dimetion or paras
  ...
  private String FunName = ""; // Function name
  private String dimshape = "";

  public DFGTranCodeGenerator() {
  ...
  public void drawStart(SimpleNode n) {
    ...
    try {
      ...
      if (flagstart == 1) {
        ...
        rf1.writeBytes("label = \"FUNCTION" + "_" + FunName + "\";\n");
      }
  ...
  public void drawVar(String iname, String outpoint) {
    try {
      ...
      if (sbt.findVar(iname)) { // if it is a dim
  ...
  public void drawDIM(String fl, int ndim2) {
    ...
    try {
      rf1.writeBytes("aa" + itos(ndim2) + " [label=\"" + fl +
                     "\", shape=ellipse];\n");
    }
    catch (Exception e) {}
  ...
  public void drawTerm(String fn, String fl, String tn, String hl) {
    ...
    try {
      if (fl != "") {
        rf1.writeBytes(fn + " [label=\"" + fl + "\"" + dimshape + "];\n");
      }
  ...
  throws DFGException
  {
    ...
    if (n.getImage() != null && n.getImage().equals("D")) {
      dimshape = ", shape=ellipse";
    }
    else {
      dimshape = "";
    }

    if (treecon == "START") { // start
      drawStart(n);
    }
    else {
      if (flagdim != "" && treecon == "ID") { // is dimesion var
        //           flagdim=0;
        int nd = sbt.accVar(n.getImage(), "L");
        drawDIM(n.getImage(), nd); // add a lable
        if (flagdim == "DIM" && flagstart == 1) { // dim in function in start
          int nv;
        ...
        flagdim = "";
        if (treecon == "DIMENSION" || treecon == "DIM" || treecon == "PARAS") { // start dimesion defin
          flagdim = treecon;
        }
        else {
          if (treecon == "WHERE") { // node is WHERE
            drawWhere();
            scope = scope + 1;
            lname = lname + " " + itos(scope);
          }
          else {
            if (treecon == "ASSIGN") { // node is ASSIGN
              flagassign = 1;
              //           drawAssign();
              nbak = n; // backup subtree
            }
            else {
              if (n.getImage() != null && treecon != "FUNCTION") { // node is terminal term (N,d,2)
                if (flagassign == 1) { // assign left
                  flagassign = 0;
                  drawAssign();
                  ...
                  if (treecon == "ID") { // is id or dimesion at assign right
                    int nv;
                    if (sbt.findVar(n.getImage())) { // is dim
                      nv = sbt.accVar(n.getImage(), "R");
                    }
                    else { // is id
              ...
              else { // node is operator or function
                if (treecon != "FUNCTION") {
...
//                  drawAssignLable(n.getImage(), "aa"+itos(nl));
    ...
    if (n.children != null) {
      ...
      for (int i = 0; i < n.children.length; ++i) { // process all children
        ...
        if (n2 != null) {
          if (drawDFG(n2) == 1) { // process each child
    ...
    if (treecon == "DIMENSION" || treecon == "DIM" || treecon == "PARAS") { // a dim branch end
      flagdim = "";

    }
    if (n.getImage() == getSname(lname)) { // a term end
      lname = getPname(lname); // cut a child

    }
    if (treecon == "WHERE") { // a "where" branch end
...
//    rf1.writeBytes("\f");
  ...
  public void genTable() { // generate a dim index table
    genTable(snode);
  }
  ...
  public void genTable(SimpleNode n) {
    ...
    /////////////////
    if (flag_d == 1) { //||(n.getImage()!=null&&n.getImage().equals("D"))) {
      // dtable.put(n.getImage(),sbt.reqN());
      if (treecon == "ID") { //||n.getImage().equals("D")) {
        int nd = sbt.accVar(n.getImage(), "L");
        if (flagstart != 1) {
          drawDIM(n.getImage(), nd); // add a lable
        }
    ...
    if (treecon == "DIMENSION" || treecon == "DIM" ||
        (treecon == "PARAS" && flagstart != 1)) {
      flag_d = 1;
...
//////////////////////////////////////////////////////////////////////
  ...
  throws DFGException
  {
    ...
    if (filename == null) {
      ...
      outfilename = "../tests/lucid/tran-" + filename + ".ipl";
      FunName = filename;
      flagstart = 0;
    }
    try {
